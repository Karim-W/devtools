...
export default function Epoch() {
	...
	return (
		<div className="w-full h-full flex flex-col justify-start items-start p-4 gap-4">
			...
			<input
				className={BASE_CSS_PROPERTIES.INPUT_BOX + " ml-2"}
				type="text"
				onChange={(e) => {
			...
			<div className="text-left text-xl font-extralight text-white">
				Local Date:
			</div>
			<CopiedTextBox text={date.toString()} />
			<div className="text-left text-xl font-extralight text-white">
				UTC Date:
			</div>
			<CopiedTextBox text={date.toISOString().toString()} />
			<div className="text-left text-xl font-extralight text-white">
				Unix Time:
			</div>
			<CopiedTextBox text={epoch} />
		</div>
	);
}
